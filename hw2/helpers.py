...
def check_type(actual, expected, checking_return_val=True):
    ...
    if checking_return_val:
        msg = (
            f"The function returned a value of the wrong type.\n"
            f"  Expected return type: {expected_type.__name__}\n"
            f"  Actual return type: {actual_type.__name__}"
        )
    else:
        msg = (
            f"The actual value has the wrong type.\n"
...
def __check_dict_keys(actual, expected):
    ...
    rmsg = (
        "  A key {} appears in the expected result\n"
    ...
    rmsg2 = (
        "  A key {} appears in the actual result\n"
...
def __check_list_length(actual, expected, desc=""):
    ...
    if desc:
        msg = (
            f"The actual length ({len(actual)}) of the {t_str} of {desc} "
            f"does not match the expected length ({len(expected)}) of "
            f"the list of {desc}"
        )
    else:
        msg = (
            f"The actual length ({len(actual)}) does not match the "
...
def check_2D_list_unstructured(actual, expected):
    ...
    for outer_idx, (actual_sub, expected_sub) in enumerate(zip(actual, expected)):
        ...
        for inner_idx, (actual_val, expected_val) in enumerate(
            zip(actual_sub, expected_sub)
        ):
...
def check_2D_general(
    check_element_type_fn, element_type_str, check_element_equality_fn, actual, expected
):
    """
    ...
    type_error_msg = (
        f"The actual value does not have the correct type.\n"
...
def check_2D_list_unmodified(
    param_name, before, after, inequality_fn=lambda x, y: x != y
):
    """
...
def check_cmyk(actual, expected):
    ...
    chk = (
        ...
        and len(actual) == 4
        and all(
            (isinstance(v, float) or isinstance(v, int)) and 0 <= v <= 1 for v in actual
        )
        and actual[0] == pytest.approx(expected[0])
        and actual[1] == pytest.approx(expected[1])
        and actual[2] == pytest.approx(expected[2])
        and actual[3] == pytest.approx(expected[3])
    )
